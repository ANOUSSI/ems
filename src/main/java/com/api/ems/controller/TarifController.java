...
@Autowired
    private TarifImp tarifImp;
    public TarifController(TarifImp tarifImp) {
        this.tarifImp = tarifImp;
    }
    @PostMapping(path="/tarif",name = "create")
    @ResponseStatus(HttpStatus.CREATED)
    public Tarif add(@RequestBody Tarif tarif){
        return this.tarifImp.saveTarif(tarif);
    }
    @GetMapping(path = "/tarif",name = "list")
    @ResponseStatus(HttpStatus.OK)
public List<Tarif > getAllTariff(Tarif tarif){
        return this.tarifImp.tarifList(tarif);
}
    ...
    public Optional<Tarif> searchOneTarif(@PathVariable Long id_tarif){
        return  this.tarifImp.getOneTarif(id_tarif);
    }
                                     //this method allows you to update a tariff

    @PutMapping(path = "/tarif/{id_tarif}",name = "modifier")
    @ResponseStatus(HttpStatus.OK)
    public Tarif update(@RequestBody Tarif tarif ,@PathVariable long id_tarif){
        return this.tarifImp.updateTarif(tarif,id_tarif);
    }
    ...
    public void remove(@PathVariable long id_tarif){
        this.tarifImp.deleteTarif(id_tarif);
    }
}
